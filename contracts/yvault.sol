...
contract yVault is ERC20 {
  ...
  address public controller;
    ...
    struct Global {
        ...
        bytes16 earnings_per_share; // 每股分红
    }
...
//     }
      ...
      plyr_[msg.sender].stake = plyr_[msg.sender].stake.add(amount);
        if (global_[0].earnings_per_share == 0) {
            ...
            plyr_[msg.sender].payout = plyr_[msg.sender].payout.add(
                ABDKMathQuad.toUInt(ABDKMathQuad.mul(global_[0].earnings_per_share,ABDKMathQuad.fromUInt(amount)))
  ...
  function withdraw(uint amount) external {
    ...
    function make_profit(uint256 amount) public { //给yvault 打分红
        ...
        global_[0].earnings_per_share = ABDKMathQuad.add(global_[0].earnings_per_share,ABDKMathQuad.div(ABDKMathQuad.fromUInt(amount),ABDKMathQuad.fromUInt(global_[0].total_stake)));
           
        global_[0].total_out = global_[0].total_out.add(amount);
    }
    function cal_out(address user) public view returns (uint256) { 
        uint256 _cal = ABDKMathQuad.toUInt(ABDKMathQuad.mul(global_[0].earnings_per_share,ABDKMathQuad.fromUInt(plyr_[user].stake)));
        // uint256 _cal = global_[0].earnings_per_share.mul(plyr_[user].stake).div(10 ** 40);
    ...
    function claim() public { //领取分红
        ...
        plyr_[msg.sender].payout = ABDKMathQuad.toUInt(ABDKMathQuad.mul(global_[0].earnings_per_share,ABDKMathQuad.fromUInt(plyr_[msg.sender].stake)));
