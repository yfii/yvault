...
contract StrategyMushroomsUSDCV1 {
    ...
    address constant public output = address(0xa283aA7CfBB27EF0cfBcb2493dD9F4330E0fd304); //MM
    ...
    address constant public mmVault = address(0x23B197dc671A55F256199cF7e8Bee77Ea2bDC16D);//主合约.
    address constant public mmFarmingPool = address(0xf8873a6080e8dbF41ADa900498DE0951074af577); //领取奖励的合约
    uint256 constant public mmFarmingPoolId = 5; //领取奖励的池子编号

    
    uint256 public strategyfee = 100;
    uint256 public fee = 300;
    uint256 public burnfee = 500;
    uint256 public callfee = 100;
    uint256 constant public max = 1000;
    ...
    constructor() public {
        ...
        getName = string(
            abi.encodePacked("yfii:Strategy:", 
                abi.encodePacked(IERC20(want).name(),"Mushrooms Finance"
                )
            ));
        swap2YFIIRouting = [output,want,weth,yfii];// mm -> usdc -> weth -> yfii
        swap2TokenRouting = [output,want];// mm -> usdc
        doApprove();
    ...
    function dosplit() internal{
        uint256 b = IERC20(yfii).balanceOf(address(this));
        uint256 _fee = b.mul(fee).div(max);
        uint256 _callfee = b.mul(callfee).div(max);
        uint256 _burnfee = b.mul(burnfee).div(max);
        ...
        if (strategyfee >0){
            uint256 _strategyfee = b.mul(strategyfee).div(max); //1%
